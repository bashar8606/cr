...
export const useHomeBanner = ({ style }) => {
  ...
  useLayoutEffect(() => {
    const ctx = gsap.context((self) => {
      ...
      let tl1 = gsap.timeline({ force3d: true, scrollTrigger: { trigger: main.current, scrub: 0.5, end: "150%", pin: true } })

      tl1.to(logo_wrap, { yPercent: -100, autoAlpha: 0, ease: Expo.easeOut, filter: `blur(5px)`, duration: 1.5 }, "<")
      tl1.to(title, { yPercent: 0, autoAlpha: 1, stagger: .05, ease: Expo.easeOut, duration: 1.5 }, "<")
      tl1.to(title, { yPercent: -100, autoAlpha: 0, stagger: .02, ease: Expo.easeOut, duration: 1.5 })
      tl1.to(title_sm, { yPercent: 0, autoAlpha: 1, ease: Expo.easeOut, duration: 1.5 }, "<")
      // tl1.to(line, { scaleY: 1, ease: Expo.easeOut, duration: 3, transformOrigin:"top top" })



      let tl2 = gsap.timeline({ force3d: true, scrollTrigger: { trigger: main.current, scrub: 0.5, start:" top bottom", end:"bottom 80%" } })
      tl2.to(line, { scaleY: 1, duration: 3, transformOrigin:"top top" }, "<")
